...
class BayesianOptimizer:
    ...
    def suggest_next_point(self):
        ...
        for starting_point in starting_points:
            res = sciopt.minimize(
                objective, starting_point, bounds=self.bounds, method="L-BFGS-B"
            )
...
if __name__ == "__main__":
    ...
    np.random.seed(42)
    X_train = np.array(
        [[np.random.uniform(bound[0], bound[1]) for bound in bounds] for _ in range(20)]
    )
    ...
    optimizer = BayesianOptimizer(
        X_train, y_train, bounds=bounds, granularity=granularity
    )
